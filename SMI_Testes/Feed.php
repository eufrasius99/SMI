...
$queryContent = "(
    SELECT
        CONTEUDO.idConteudo,
        conteudo.idTipoConteudo,
        CONTEUDO.idUtilizador,
        CONTEUDO.titulo,
        CONTEUDO.dataHora,
        CONTEUDO.descricao,
        conteudo.thumbnailPath,
        CONTEUDO.publico
    FROM
        `$dataBaseName`.`conteudo`
    JOIN `$dataBaseName`.`subscricao_categoria`,
        `$dataBaseName`.`categoria`,
        `$dataBaseName`.`subcategoria`,
        `$dataBaseName`.`conteudo_sub_categoria`
    WHERE
        categoria.idCategoria = subcategoria.idCategoria AND conteudo_sub_categoria.idSubCategoria = subcategoria.idSubCategoria AND conteudo_sub_categoria.idConteudo = conteudo.idConteudo AND(
            subscricao_categoria.idSubscritor = $idUserSession AND subscricao_categoria.idCategoria = categoria.idCategoria
        )
    ORDER BY
        conteudo.dataHora
    DESC
...
UNION
    ...
    FROM
        `$dataBaseName`.`conteudo`
    JOIN `$dataBaseName`.`subscricao_utilizador` WHERE subscricao_utilizador.idPublicador = conteudo.idUtilizador AND subscricao_utilizador.idSubscritor = $idUserSession
    ORDER BY
        conteudo.dataHora
    DESC
);";
...
<?php include 'BStrap.css'; ?>
    ...
    <body>
        ...
        <main>
            <h1  class="fw-light" style="text-align: center;padding-top:30px;padding-bottom:30px">Feed</h1>
